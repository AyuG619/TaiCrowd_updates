...
@ti.data_oriented
class SF_People:
    def __init__(self, config , scene = None):#change
        self.config = config
        # self.N = config.N
        
        # self.scene = Scene(config)
        
        #####change to self.scene
        if scene is not None:
            self.scene = scene 
        else:
            self.scene = Scene(config)    
        
        if hasattr(self.config, 'scene') and self.config.scene == "circle":
    # Make sure group sizes match total N
            half_n = self.config.N // 2
            self.config.group = [half_n, half_n]  # [100, 100] for N=200
            print(f"DEBUG: Fixed circle group = {self.config.group}")
        ...
        # self.belong_batch = ti.field(ti.i8,self.config.N) previously
        self.belong_batch = ti.field(ti.i16,self.config.N) #new
        ...
        ####the change####
        if isinstance(self.config.group ,(list, tuple, np.ndarray)) and len(self.config.group)>0:
            group_count_size=len(self.config.group)
        else:
            group_count_size=1    
        group_count_size =  max(1,group_count_size)    
        self.group_count = ti.field(ti.i32,group_count_size)
        ############
        self.fill_parm()

        # QUICK FIX: Force create gender array for circle scene
        if  hasattr(self.config, 'scene') or self.config.gender == "circle":
            if not hasattr(self.config, 'gender') or len(self.config.gender) == 0:
                self.config.gender = []
                for i in range(self.config.N):
                    if i < self.config.N // 2:
                        self.config.gender.append(0)  # Blue
                    else:
                        self.config.gender.append(1)  # Red
        
        if hasattr(self.config, "gender") and self.config.use_gender == 1:
            genders_np = np.array(self.config.gender, dtype=np.int8)
            if len(genders_np) == self.config.N:
                self.gender.from_numpy(genders_np)
                print("Gender field populated successfully")
            else:
                print(f"[Warning] gender array length {len(genders_np)} != N ({self.config.N})")   
        ...
        # self.groups = ti.field(ti.i32, shape=config.N)
        # self.colors = ti.field(ti.i32, shape=config.N)
    ...
    def init_agents(self,config):
        if hasattr(config, "spawn_points") and hasattr(config, "groups"):
            spawn_points = np.array(config.spawn_points, dtype=np.float32)
            groups = np.array(config.groups, dtype=np.int32)

    # Initialize positions
            self.pos.from_numpy(spawn_points)
    # Initialize groups
            self.groups.from_numpy(groups)  
            color_array = np.zeros(config.N, dtype=np.int32)       
            for i in range(config.N):
                if groups[i] == 0:
                    color_array[i] =  0x0000FF  # Red
                else:
                    color_array[i] = 0xFF0000   # Blue
                
            self.colors.from_numpy(color_array)    
        else:
            print("[Warning] No spawn_points/groups in config. Using default positions.")            
    ...
    def fill_parm(self):
        ...
        """
        print(f"=== FILL_PARM DEBUG ===")
        print(f"config.scene = {getattr(self.config, 'scene', 'NOT SET')}")
        print(f"config.N = {self.config.N}")
        print(f"config.batch = {getattr(self.config, 'batch', 'NOT SET')}")
        print(f"config.group = {getattr(self.config, 'group', 'NOT SET')}")
        print(f"config.use_gender = {getattr(self.config, 'use_gender', 'NOT SET')}")
        print(f"len(config.gender) = {len(getattr(self.config, 'gender', []))}")
        ...
        print(f"DEBUG: config.group = {self.config.group}")
        print(f"DEBUG: config.N = {self.config.N}")
        print(f"DEBUG: type(config.group) = {type(self.config.group)}")
    
        
        ###the change######
        # self.group_count.from_numpy(np.array(self.config.group))
        
       # Handle both list and int group configurations
        if isinstance(self.config.group, (list, tuple, np.ndarray)):
            group_array =  np.array(self.config.group, dtype=np.int32)
        else:
            group_array = np.array([self.config.group], dtype=np.int32)  # Single group
           
        self.group_count.from_numpy(group_array)
        ########################    
        ...
        ############
        if isinstance(self.config.group, (list, tuple, np.ndarray)):
            for batch in range(len(self.config.group)):
                group.append([batch] * self.config.group[batch])
        else:
            # For single int group, all agents belong to group 0
            group.append([0] * self.config.group) 
    ...
    def render(self, gui):
        ...
        for i in range (self.config.N): 
            ...
            #         _color = 0xFF0000   # Red
            #     else:
            #         _color = 0x0000FF   # Blue
            
            if self.config.use_gender == 1 and hasattr(self, "gender"):
                if self.gender[i] == 0:      # first gender group
                    _color = 0x0000FF        # Blue
                else:                        # second gender group
                    _color = 0xFF0000        # Red
            else:
                _color = 0x002300            # default green if gender not used
    ...
    def ggui(self, gui):
        ...
        if self.config.im is None:
            ...
            gui.set_image(self.config.im)
           #new part start 
        if (hasattr(self.config, 'scene') and self.config.scene == "circle" and 
        self.config.use_gender == 1 and hasattr(self, "gender")):
            male_positions=[]
            female_positions=[]
            positions = self.pos.to_numpy()
            for i in range(self.config.N):
                if self.gender[i]==0:
                    male_positions.append(positions[i])
                else:
                    female_positions.append(positions[i])    
             
            if male_positions:
                male_positions = np.array(male_positions)
                gui.circles(male_positions, color=(0,0,1),radius=0.02)
            if female_positions:
                female_positions = np.array(female_positions)
                gui.circles(female_positions,color=(1,0,0),radius=0.02)
        else:
            #new part end 
    ...
    def update(self):
        for i in range(self.config.N):
            ...
            # if close enough to targetÃ¯Â¼Å’stop
