...
class Scene:
    def __init__(self, config):
        ...
        if config.dynamic_search == 0: #static memory allocation
            # Record the number of people in each grid
            ...
            self.particle_id = ti.field(dtype=ti.i32, shape=config.N*4, name="particle_id") #change
        else:
            # Record the total number of people in each grid
            self.grid_count = ti.field(ti.i8, self.pixel_number)
            # Record the index of people in each grid
            self.grid_matrix = ti.field(ti.i8)
            self.block = ti.root.dense(ti.i, self.pixel_number)
            pixel = self.block.dynamic(ti.j, config.N)
            pixel.place(self.grid_matrix)

        # obstacle_setting = 1
        # Record whether there are obstacles in each grid (static)
        self.obstacle_exist = ti.field(ti.f32, shape=(self.window_size, self.window_size))
        # For obstacle grids, record the center position of each grid (static, sparse)
        ...
        if hasattr(config, "scene") and config.scene == "long_hall_gendergroup":
            self.genders = []
            half = self.window_size / 2
            for (x, y) in config.spawn_points:  # spawn_points must be defined in config
                if x < half:
                    self.genders.append(0)  # Left side â†’ gender 0 (blue)
                else:
                    self.genders.append(1)  # Right side â†’ gender 1 (red)

            # Make this available to SF_People
            config.gender = self.genders    
        #DEBUG CHECK
        print(f"=== SCENE INIT DEBUG ===")
        print(f"hasattr(config, 'scene'): {hasattr(config, 'scene')}")
        if hasattr(config, 'scene'):
            print(f"config.scene: {config.scene}")
        
        
    #     #for circle
        if hasattr(config, "scene") and config.scene == "circle":
            print("CALLING init_circle()")
            self.init_circle(config)  
        else:
            print("NOT calling init_circle() - scene check failed")    
            
            
    
    # #for circle
    def init_circle(self,config):
        N=config.N
        radius = self.window_size // 3
        ...
        for i in range(N):
            ...
            spawn_points.append((x / self.window_size, y / self.window_size))
        ...
        config.gender = genders   
        config.use_gender = 1  
        config.batch  = 2
        config.group = [N // 2, N // 2] 
        
        print(f"=== INIT_CIRCLE DEBUG ===")
        print(f"Set N = {N}")
        print(f"Set batch = {config.batch}")
        print(f"Set group = {config.group}")
        print(f"Set use_gender = {config.use_gender}")
        print(f"Set len(gender) = {len(config.gender)}")
