...
class Config:
    def __init__(self):
        ...
        self.max_neighbour = 5 #previously 10 
        ...
        # render settings
        self.people_radius = 1 #previously 3
    ...
    # def fill_group(self):
    #     """Fill batch by evenly splitting N agents into self.batch groups."""
    #     self.group = []
    #     base_size = self.N // self.batch
    #     remainder = self.N % self.batch
    ...
    def fill_group(self):
        ...
        self.group = []
        self.batch = self.group_target.shape[0] #æœ‰å‡ æ‰¹äºº
        sum = 0
    ...
    def shuffle_group(self,target):
        ...
        self.batch = count
        self.group_target = np.array([target]*self.batch) #æ¯æ‰¹äººçš„ç›®çš„åœ°
        self.group_vel = np.array([[0.01, 0.]]*self.batch) #æ¯æ‰¹äººçš„åˆå§‹é€Ÿåº¦
    ...
    def set_scene(self,value):
        ...
        if value == "circle":
            ...
            self.group_target = np.zeros(shape=(self.N, 2), dtype=np.float32)
            self.group_vel = np.zeros(shape=(self.N, 2), dtype=np.float32) #é»˜è®¤æ¯ä¸ªäººçš„target åˆé€Ÿåº¦éƒ½ä¸åŒ
    ...
    def set(self,key,value):
        ...
        if key == "n":
            ...
            self.pos_0 = np.zeros(dtype=np.float32,shape=(self.N, 2))
            self.desiredpos_0 = np.zeros(dtype=np.float32, shape=(self.N, 2))
            self.desiredvel = np.zeros(dtype=np.float32, shape=self.N)  # æœŸæœ›é€Ÿåº¦å¤§å°m/s  
            self.speed_range = 15 #20240408:äººç¾¤çš„é€Ÿåº¦å·®å¼‚å€¼ï¼Œåœ¨60ä¹‹ä¸ŠåŠ å‡è¿™ä¸ªèŒƒå›´çš„éšæœºæ•°      
